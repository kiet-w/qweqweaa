        ...
        <div className="flex flex-col gap-lg">
          <div className="card">
            <div className="flex justify-between align-center mb-md pb-sm" style={{ borderBottom: '1px solid var(--border-primary)' }}>
              ...
              <span className={`badge ${getOrderBadgeClass(order.status)}`} style={{ fontSize: '12px' }}>
                  ...
                  {order.items?.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td style={{ textAlign: 'right' }}>
                        {item.product?.price ? `${item.product.price.toLocaleString()} VNĐ` : '--'}
                      </td>
      ...
      {isAssignModalOpen && (
        <div className="modal-overlay" onClick={() => setIsAssignModalOpen(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <h3 className="m-0 mb-md">Gán Shipper cho Đơn #{order.id}</h3>
